...
function Salon() {
  const { services, addService, removeService, fetchServices, isAddingService } = useSalonStore();
  ...
  // Handle input changes (text & file separately)
  const handleChange = (e) => {
    if (e.target.name === "image_url") {
      setFormData({ ...formData, image_url: e.target.files[0] }); // Store file
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsModalOpen(false); // Close modal only after adding is completed
  };
  ...
  return (
    <div className="p-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg w-96 relative">
            {/* Close Button (disabled if adding service) */}
            {!isAddingService && (
              <button
            ...
            <h3 className="text-xl font-semibold mb-4">Add New Salon Service</h3>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <input type="text" name="category" value={formData.category} onChange={handleChange} placeholder="Category" className="p-2 rounded bg-gray-700 text-white" required />
      ...
      {/* Display Total Salon Services */}
      <div className="bg-gray-900 p-6 rounded-lg">
        <h3 className="text-xl font-semibold mb-4">Total Salon Services</h3>
          ...
          {services.map((service) => (
            <li key={service._id} className="flex justify-between items-center bg-gray-800 p-4 rounded">
              <div>
                ...
                {service.duration && <p className="text-gray-400">Duration: {service.duration} min</p>}
              </div>
