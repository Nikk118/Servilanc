...
const getAcceptedBooking = asyncHandler(async (req, res) => {
  const professional = req.professional._id;
  const acceptedBookings = await Booking.find({ professional: professional , status:"Accepted" });
  return res.status(200).json({ acceptedBookings });
});
